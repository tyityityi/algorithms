...
1. 计算要 put 的 key 的位置，获取指定位置的 Segment。
    ...
    //把key计算得来的 hash 值无符号右移segmentShift位（初始化时获得），然后与 segmentMask=15 做与运算
    // 其实也就是把高4位与segmentMask（1111）做与运算
...
抽屉原理 9个抽屉十个苹果 必然有一个抽屉里有两个。

解决： 1、hash 长文本也能高效计算hash值。

​			2、不能逆推回原文。

​			3、输入不同 Hash值也要不同。

​			4、尽可能分散，table中slot大部分处于空闲状态眼尽可能降低冲突。

### 2、HashMap中存储结构	

在jdk1.8中 HashMap以 链表+数组+红黑树构成，每一个数据单元都是Node结构，Node中包含Key、Value、Next、Hash字段。

### 3、创建HashMap没有指定数组长度 初始长度是多少？负载因子是多少？有什么作用

初始长度是16 ， 负载因子是0.75。

负载因子作用：计算扩容阈（yu）值，比如调用默认无参构造时，
...
如不满足，不会转化为树，只会发生一次**resize**，**散列表扩容**

### 6、Node对象内有hash字段，这个值与key对象的hashcode（）返回值一样么？

不是的，这个值是hashcode()的值二次加工得到的

加工原则：key的hashcode高16位(无符号右移16位)^（异或）低16位(key.hashcode())得到的新值

原因：hash寻址算法（hash&（与）（table.length-1））的缘故

1、散列表长度必须是2的次方

2、因寻址算法，table的length的二进制高位为（1）其余全是（0）

如：16 -->1 0000 32–>10 0000

table.length-1转化为二进制：15–>1111 31–>1 1111

再加上下标为0 这样可以保证数组长度是2的次方
...
主要分成四种情况，先寻址,根据key.hashcod经过高低位异或得到hash值,计算hash值与&(table.length-1) 得到槽位下标slot

#### 1、slot=null。

直接占用slot，把key、value包装成node对象直接放在slot中

#### 2、slot！=null

##### 2.1引用的node还没有链化

对比当前的node对象key与put对象的key值是否相等

如果相等：进行replace操作，就是替换操作

不相等：就是hash冲突，在node后面追加，1.8采用**尾插法**, 1.7头插

##### 2.2引用的node已经链化

和上一种方法很相似，只是多了一个遍历链表 没有找到就**插入尾部**

重点：需要检查**链表长度**是否达到**树化阈值**，达到就需要树化操作

##### 2.3引用的node冲突严重已经成为红黑树

TreeNode继承node 加了parent left right red

红黑树满足二叉树的所有特性 找到他的父节点

情况1：一直向下查找 直到查询到叶子节点还是没找到 此时插入节点就是父节点所在的左节点或右节点，插入操作可能会破坏红黑树的平衡， 所以需要进行红黑树的平衡算法

情况2：在啊向下查找的过程中，找到了与之相同的key之，之后就是进行一次 replay的操作，进行值得替换
...
3、从叶子结点到根节点的路径上，他的每条路径的黑色节点都是一样的数量**（黑高）；**

4、父子节点不能同时为红色；

5、插入时，插入的节点为红色；黑色一定会失衡 红色有可能不失衡

6、每个叶子节点是黑色的；

### 10、jdk8为什么引入红黑树

解决hash碰撞导致的链化严重问题，严重影响了get查询效率，因为链表的查询非常慢时间复杂度O（n）

### 11、为什么链表查询效率就变低了

因为链表不像数组地址是连续的，在内存中他是依靠指针查询的，他不能按下标去直接查找，要按指针一个一个的查找，要是在末尾，要一个一个找，非常耗费性能；

### 12、Hashmap的扩容机制

#### 1、为什么会扩容

写入数据过多时会扩容，在hashMap中有一个记录当前数据量字段的，当达到阈值之后会进行扩容；

#### 2 、扩容后会达到多大？算法是啥

table长度必须是2的次方数，所以是按照上一次tableSize做位运算得到的

位移一位 扩容两倍

#### 3、为什么采用位运算 而不是直接乘2

性能方面 cup不支持乘法运算，最后都是加法运算，效率很低

#### 4、新创建的数组，老数组中的数据怎么迁移？
...
没有链化表明next为null，slot没有发生冲突，直接迁移就好，根据**新表的tableSize计算新位置**，放过去就好了。

##### 3、存的是Node已链化

说明发生过冲突，需要把当前这个node的链表**拆分成两个链表，分别是高位链和低位链**；

高位链和低位链：因为老表中桶位已经链化了，能推出链表中所有node的hash字段转化为二进制之后**低位都是相同的**

就是老表的tableSize-1 ，如size=16的 二进制就是16-1=15=0 1111；这些链表低位相同，但是高位（第五位）不一定相同，有的是1有的是0（0 1111,1 1111），这既是高位连和低位链，他们存放到新的数组中的位置也不一样。

**低位链迁移到新表的下标（slot）是和老表一样的，**

**高位连迁移就是老表的下标加上老表的数组长度，如老表下标为8长度16，则新表低位链为8，高位链下标8+16=24**

##### 4、存的红黑树的跟节点TreeNode

Trenode中还保留着next字段，其实它内部还维持着一个链表，只不过查询是不用他，新增或删除是还要维护这个链表.
...
## ArrayList, HashMap, HashSet为什么线程不安全

https://blog.csdn.net/qq_39579087/article/details/120305051
