...
泛型本质是将数据类型参数化，它通过擦除的方式来实现。声明了泛型的 .java 源代码，在编译生成 .class 文件之后，泛型相关的信息就消失了。
...
Java的泛型是**伪泛型**，这是因为在jdk的javac**编译成.class文件时**，所有的**泛型信息都会被擦掉** 。
...
| 基本类型     | (比特位)位数bit | 字节byte | 默认值  | 包装类型  |
| ------------ | --------------- | -------- | ------- | --------- |
| `int`        | 32              | 4        | 0       | Integer   |
| `short`      | 16              | 2        | 0       | Short     |
...
1. Java 里使用 `long` 类型的数据一定要在数值后面加上 **L**，否则将作为整型int解析。
...
> 《深入理解 Java 虚拟机》 ：**局部变量**表主要存放了编译期可知的基本数据类型**（boolean、byte、char、short、int、float、long、double）**、**对象引用**（reference 类型，它不同于对象本身，可能是一个指向对象起始地址的**引用指针**
...
- 语法：
    - **抽象类**Abstract：方法可以有抽象的，也可以有非抽象（有具体实现）, 有构造器
        ...
        如果将一个类设置为abstract，此类不可生成对象，**必须被继承使用**。
...
多实现(接口)：Person **implements** IRunable,IEatable（符合多项国际化标准）
...
#### NIO (Non-blocking(linux)/New I/O(java)) 多路复用模型
...
异步 IO 是基于事件和回调机制callback实现的，也就是应用操作之后会直接返回，不会堵塞在那里，当后台处理完成，操作系统会通知相应的线程进行后续的操作。
...
### !!!通过反射创建创建对象(ioc动态创建bean实例对象 )

<img src="imgs/image-20211005211317258-3439599.png" alt="image-20211005211317258" style="width:50%;" />
